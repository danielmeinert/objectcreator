...
import pathgenerator.template as templ
...
class PathObject:
    ...
    def generateObject(self, template: templ.PathTemplate, settings_input: dict):
        ...
        for i in range(num_images):
            ...
            offset_x = -32
            offset_y = -15 if template.is_small else 0
            offset_y -= 8 if settings['raised'] else 0
            sprite = spr.Sprite(image, auto_offset_mode=None, offset=(offset_x, offset_y))
...
class PathGenerator:
    def __init__(self, fix_mask, main_window):
    ...
    def loadSettings(self):
        ...
        self.settings = {}
        self.settings['author'] = self.main_window.settings.get('author', '')
        self.settings['author_id'] = self.main_window.settings.get('author_id', '')
        self.settings['no_zip'] = self.main_window.settings.get('no_zip', False)
        self.settings['version'] = self.main_window.settings.get('version', '1.0')
    ...
    def loadTemplatesAtStart(self):
        self.templates = {}
        for file in listdir(aux.resource_path("pathgenerator/templates")):
            if file.endswith(".template"):
                template = templ.PathTemplate.fromFile(f'{aux.resource_path("pathgenerator/templates")}/{file}')
                if template:
