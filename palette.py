...
class Palette(np.ndarray):
    ...
    def getColor(self, color : str):
        if color not in self.color_dict.keys():
            raise KeyError('Color not in palette')
        if color != 'Sparkles':
...
ingame_remaps = Palette(np.load('remap_strips.npy'), remapColors(), has_sparkles = False)
...
def switchPalette(image : Image.Image,pal_in : Palette ,pal_out : Palette, include_sparkles = True):
    ...
    if include_sparkles and not (pal_in.has_sparkles and pal_out.has_sparkles):
        raise ValueError('Asked to include sparkles but one of given palette has no sparkles.')
...
def addPalette(image,palette : Palette = orct,dither = True, include_sparkles = False):
    ...
    if include_sparkles and palette.has_sparkles:
        palette = np.concatenate((np.array(palette),np.array([palette.sparkles])))
    ...
    p = Image.new("P", (1,1))
    p.putpalette(pal_in)
    return image.quantize(method=3, palette=p, dither = int(dither)).convert('RGBA')
